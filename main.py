...
class PDFReader(QMainWindow):
    ...
    def __init__(self):
        ...
        self.toc.setFont(QFont("", 20))  # 目录文字的字体及其大小控制，修改字体将字体名放入双引号中（为空表示使用默认字体），字体大小修改数字即可，数字越大字体越大
        self.file_path = ""
        self.page_num = 0
        self.doc = None
        ...
        self.input_dialog = QLineEdit(self)
        ...
        qlayout = QVBoxLayout()
        btn_3 = QPushButton("搜索")
        button_layout = QHBoxLayout()
        btn_3.clicked.connect(self.sFile)
        button_layout.addWidget(self.input_dialog, 7)
        button_layout.addWidget(btn_3)
        bhwg = QWidget()
        bhwg.setLayout(button_layout)
        qlayout.addWidget(bhwg)
        qlayout.addWidget(self.listView)
        vwg = QWidget()
        vwg.setLayout(qlayout)
        self.layout.addWidget(vwg)
    ...
    def clickedlist(self,qModelIndex):
        ...
        filename = os.path.join(path,self.qList[qModelIndex.row()])
        self.onOpen(filename)

    # def sync_func(self):
    #     print(self.verticalScrollBar().value())
        # if(self.verticalScrollBar().value()==self.verticalScrollBar().value()):
        #     self.page_num += 1
    def sFile(self):
        files=os.listdir(path)
        result=[]
        s=self.input_dialog.text()
        for f in files:
            if s in f:
                result.append(f)
        self.qList = result
    ...
    def generateOutfile(self):
        ...
        self.tofileConnect(toDocx, toTXT)
    ...
    def generateToolBar(self):
        ...
        # sFlie.triggered.connect(self.sFlie)
        self.tofileConnect(toDocx,toTXT)
        self.pageConnect(deletePage, extractPage, insertPage)
        ...
        self.toolbar.addSeparator()
        self.toolbar.addActions([toTXT, toDocx])
        self.toolbar.addSeparator()
    ...
    def tofileConnect(self, toDocx, toTXT):
    ...
    def enlargepage(self):
        self.trans_a += 10  # 每次放大增加5%,修改此参数时注意和下方的tran_b保持一致，否则图片会变形
        self.trans_b += 10
        self.trans = fitz.Matrix(self.trans_a / 100, self.trans_b / 100).preRotate(0)
        self.generatePDFView()
